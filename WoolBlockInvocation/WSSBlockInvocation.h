...
/* Create an object that can invoke each Block in blocks in turn with the same
 * set of arguments, storing any return values for later access.
...
/* Append block to the list of Blocks; an exception will be raised if the 
...
/* Copy the contents of arg to be used as the argument at the specified index.
...
/* Copy the argument at the specified index into the space pointed to by arg,
 * which should be large enough to hold the value.
 */
- (void)getArgument:(void *)arg atIndex:(NSInteger)idx;

/* Copy the return value from the specified index into buffer, which should
 * be large enough to hold the value. Use blockReturnLength to determine the
 * appropriate size. 
